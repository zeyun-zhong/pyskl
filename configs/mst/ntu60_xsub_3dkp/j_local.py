...
train_pipeline = [
    ...
    dict(type='SampleFrames', clip_len=clip_len, frame_interval=sample_rate, out_of_bound_opt='repeat_last', keep_tail_frames=True, num_clips=1),
...
val_pipeline = [
    dict(type='PreNormalize3D'),
    dict(type='GenSkeFeat', dataset='nturgb+d', feats=['j']),
    dict(type='SampleFrames', clip_len=clip_len, frame_interval=sample_rate, out_of_bound_opt='repeat_last', num_clips=1),
    dict(type='PoseDecode'),
    dict(type='FormatGCNInput', num_person=2, mode=mode),
    dict(type='Collect', keys=['keypoint', 'label'], meta_keys=[]),
    dict(type='ToTensor', keys=['keypoint'])
...
test_pipeline = [
    dict(type='PreNormalize3D'),
    dict(type='GenSkeFeat', dataset='nturgb+d', feats=['j']),
    dict(type='SampleFrames', clip_len=clip_len, frame_interval=sample_rate, out_of_bound_opt='repeat_last', num_clips=10),
    dict(type='PoseDecode'),
    dict(type='FormatGCNInput', num_person=2, mode=mode),
    dict(type='Collect', keys=['keypoint', 'label'], meta_keys=[]),
    dict(type='ToTensor', keys=['keypoint'])
